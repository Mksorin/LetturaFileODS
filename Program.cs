...
namespace LetturaFileODS
{
    class Program
    {
        static void Main(string[] args)
        {
            string nome_input = args[0];
            FileInfo fileInput = new FileInfo(nome_input);
            DataSet osdFile = new OdsReader().ReadOdsFile(fileInput.FullName);                    

            //We open a streamwriter to write the ods file in a csv format.
            //This process can be avoided if you are ok with working on DataTables.
            using (StreamWriter sw = new StreamWriter(Path.Combine(fileInput.DirectoryName, fileInput.Name.Replace(fileInput.Extension, ".csv"))))
            {
                foreach (DataTable table in osdFile.Tables)
                {
